...
<section class="mx-4 mt-8 flow-root space-y-4 sm:mx-0">
	<h2 class="text-center text-2xl font-bold">🏆 Leaderboard</h2>
	<div class="-mx-4 -my-2 sm:-mx-6 lg:-mx-8">
		<div class="inline-block min-w-full space-y-2 py-2 align-middle sm:px-6 lg:px-8">
			{#each players.sort((a, b) => b.totalScore - a.totalScore) as player, i (player.profileId)}
				<div
					class="grid grid-cols-[auto_auto_1fr_auto] items-center gap-4 rounded-lg border p-4 shadow-sm transition-all dark:border-slate-600 dark:bg-slate-800/80 dark:hover:bg-slate-700/50"
					animate:flip={{ duration: 300 }}
				>
					<div class="w-12 text-center">
						<span class={`text-xl font-bold ${getRankColor(i + 1)}`}>#{i + 1}</span>
					</div>
					<div>
					...
					<div class="min-w-0">
						...
						<div class="truncate text-sm text-slate-400">{getUsername(player)}</div>
					</div>
