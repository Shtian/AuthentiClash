...
<header>
	<div class="mx-auto flex max-w-2xl flex-col gap-y-2 sm:px-6 lg:px-8">
		<h1 class="tracking-tigh text-3xl leading-tight font-bold text-foreground">
			Joining {data.gameName}
		</h1>
		<p class="text-sm text-pretty text-muted-foreground" title={data.endsAt}>
	...
	<div class="mx-auto mt-5 max-w-2xl py-5 sm:px-6 lg:px-8">
		<form method="POST" action="?/joinGame" use:enhance={handleNewPlayer}>
			...
			<div class="col-span-3">
				<label for="nickname" class="block text-sm leading-6 font-medium text-foreground"
				...
				<div class="mt-2">
					<div class="relative flex">
						<input
							...
							required
							minlength="3"
							class="flex-1 rounded-md border-b bg-transparent px-4 py-1.5 text-lg text-foreground shadow-xs ring-1 ring-foreground/10 transition-colors hover:bg-muted focus:ring-[3px] focus:ring-ring focus:outline-none md:text-4xl"
						/>
						<button
						...
						>
							...
							<Shuffle
								class="size-6 origin-center cursor-pointer text-foreground [animation-duration:1s] [animation-iteration-count:1] {recentRefresh
									? 'animate-pulse'
									: null}"
							/>
						</button>
				...
				{#if data.classes.length > 0}
					<div class="mt-8 space-y-4">
						<p class="block text-sm leading-6 font-medium text-foreground">
